...
double simpson(vec f, int a, int b) {
	double h, k, res = 0;
	int i = 0;
	
    k = f(a) + 4 * f(a+1) + f(b-1) + 4 * f(b-2);
...
double guess(double x, void *params) { //distribuzione di densità iniziale
    ...
    double rho_plus = 3. / (4. * M_PI * radius * radius * radius);
...
double coulomb(double x, void *params) { //porzione coulombiana del potenziale
    ...
    schrodinger *D = (schrodinger *)params;
    double radius = D->sphere_radius;
    double rho_plus = 3. / (4. * M_PI * RS_Li * RS_Li * RS_Li);
    int i;
    double ext, pot1, pot2;
        ...
        if( x <= radius ) {
            ...
            ext = - 4./3. * M_PI * rho_plus * radius * radius * radius/x;
        }
...
////OK
//double Hsupp1(double r, void *params) {

//    schrodinger *D = (schrodinger *)params;
//    //int i = (r/STEP);
//    double x = D->rho(r/STEP);

//    return x * r;
//}

////OK
//double Hsupp2(double r, void *params) {
...
//    pot1 = simpson(D->rho, RMAX/STEP);
//    //pot1 = gsl_integration_glfixed (&F, x, RMAX, D->w);
//    //pot1 += (1./(x+STEP)) * gsl_integration_glfixed (&G, 0, x, D->w);

//    return 4. * M_PI * pot1;
//}

//double matrix_diagonal(double x, schrodinger *D) { //funzione per riempire la diagonale principale: potentiale + 1/(delta x)^2
...
int main() {
   ...
   for(double x = 0; x < RMAX; x+=STEP) {
        D.rho((int)(x/STEP)) = exp(-x);
    }

   double result = simpson(D.rho, 0, (int)(5/STEP));
...
//    D.w = gsl_integration_glfixed_table_alloc(250);
//    D.dim = RMAX/STEP;
...
//    vec psi, psi2, rho, dpsi2, tmp, rho_tmp, rho_guess;

//    /*= (double *)malloc((D.dim) * sizeof(double)); //funzione d'onda
