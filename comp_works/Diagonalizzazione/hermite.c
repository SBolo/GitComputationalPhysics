...
double potential(double x, void *params) { //potenziale di Malfliet-Tjon
    ...
    return (2.38/x) * ( 7.39 * exp(-3.110 * x) - 3.22 * exp(-1.554 * x) ) + l * (l + 1)/(2. * x * x);
}
...
double simpson(gsl_function *F, gsl_function *V, prms *P, int i, int j, double a, double b) {
	...
	int k = 0, t = 0, N = 5e3; //N = numero di campionamenti
	double f[N];
...
int main() {
    ...
    double x, integ, integ2, energy, energy1, energy2, energy3, df1, df2, df3, df4, df5, df6, norm[100];
    unsigned int i;
    int j, ex;
    ...
    sprintf(filename, "wave_lb%d_ls%d",P.k,P.l);
    ...
    P.dim = 5; //modificare qui per cambiare la dimensione della matrice
