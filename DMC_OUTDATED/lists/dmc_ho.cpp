...
#define WALKERS (100) //number of walkers
#define NFIX (100.) //mean number of walkers I want at convergence
...
double MonteCarlo::local_energy(double *Rx, double *Ry, double *Rz) {
    ...
    for( int i = 0; i < particles_num; i++ ) {
        ...
        sum += 3.*a - 2.*a*a*r*r/*-2. * a * a * r * r + a * (2*b*r + 1) - b*b/2.*/ + 1./2.*r*r; //nabla^2f/f+V_HO
    }
...
int main() {
    ...
    for( M->prms[0] = 0.55; M->prms[0] <= 0.7; M->prms[0] += STEPa ) {
