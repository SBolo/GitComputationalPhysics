...
#define PARTICLE_NUM (80) //number of particles
...
#define TIMEV (25.) //propagation time for VMC
#define TMINV (25.) //minimum time after the energy sampling begins
...
class MonteCarlo {
    ...
    private:
        ...
        double denst(double *, double *, double *, double *, double *, double *);
        double a_rate;
        int m_step;
        double len;
        int accepted;
...
double MonteCarlo::psit(double *Rx, double *Ry, double *Rz) {
    double sum = 0;
    double r2;
    double a = prms[0];
...
double MonteCarlo::denst(double *x, double *y, double *z, double *x2, double *y2, double *z2) {
    double sum = 0;
    double r2;
    double a = prms[0];

    for( int i = 0; i < particles_num; i++ ) {
        r2 = ( x[i]*x[i] - x2[i]*x2[i] ) + ( y[i]*y[i] - y2[i]*y2[i] ) + ( z[i]*z[i] - z2[i]*z2[i] );
        sum += r2;
    }
...
void MonteCarlo::diffuseNoBranch() { //variational MC
    ...
    for( int j = 0; j < particles_num; j++ ) {
        ...
        //psi_new = psit(pop.x, pop.y, pop.z);
        //psi_new *= psi_new;

        alpha = denst(pop.x, pop.y, pop.z, x,y,z); //psi_new/pop.psi_old;
