...
#define TIMEV (500.) //propagation time for VMC
...
class MonteCarlo { //everything about Monte Carlo
    ...
    private:
        ...
        double psi_t(double *, double *, double *); //needed for VMC
};
...
double local_energy(double *Rx, double *Ry, double *Rz, double *var_param, int particle_num) {
    ...
    for( int i = 0; i < particle_num; i++ ) {
        r = Rx[i];// + Ry[i]*Ry[i] + Rz[i]*Rz[i] );
        sum += -2. * a * a * r * r + a * (2*b*r + 1) - b*b/2. + 1./2.*r*r; //nabla^2f/f+V_HO
    }
...
double pseudo_force(double *Rx, double *Ry, double *Rz, double *var_param, int particle, int direction, int particle_num) {
    ...
    if( direction == 0 ) {
        return ( b - 2. * a * Rx[particle] ) * DTAU;

    } else if (direction == 1 ) {
        return ( b - 2. * a * Ry[particle] ) * DTAU;

    } else if( direction == 2 ) {
        return ( b - 2. * a * Rz[particle]) * DTAU;
...
double MonteCarlo::psi_t(double *Rx, double *Ry, double *Rz) {
    double a = prms[0];
    double sum = 0.;
...
void MonteCarlo::diffuseNoBranch() { //variational MC
    ...
    double en_old, en_new, pf_new, pf_old, psi_new, psi_old, rn[particles_num], rn2, alpha, A;
    int count = 0, count2 = 0;
    double upper_exp = 0., bottom_exp = 0., loop_num = (1.*(TIMEV)/DTAU);
    ...
    for( int i = 0; i < walkers_num; i++ ) { //for every walker
        ...
        psi_old = psi_t(population[i]->x,population[i]->y,population[i]->z) *
                psi_t(population[i]->x,population[i]->y,population[i]->z);

        for( int j = 0; j < particles_num; j++ ) {
            rn[j] = gauss_random_generator(dtau);

            pf_old = pseudo_force(population[i]->x,population[i]->y,population[i]->z,prms,j,0,particles_num);
            bottom_exp += (rn[j] + pf_old) * (rn[j] + pf_old);

            population[i]->x[j] += rn[j];

            pf_new = pseudo_force(population[i]->x,population[i]->y,population[i]->z,prms,j,0,particles_num);
            upper_exp += (rn[j] + 0.5*pf_new) * (rn[j] + 0.5*pf_new);
        }

        psi_new = psi_t(population[i]->x,population[i]->y,population[i]->z) *
                psi_t(population[i]->x,population[i]->y,population[i]->z);

        alpha = (psi_new/psi_old) * ( exp(-upper_exp/dtau) / exp(-bottom_exp/dtau) );
        ...
        if( thermalized ) {
            ...
            n += /*population[i]->weight * */en_new / loop_num;
            d += 1./*population[i]->weight*/;
...
void MonteCarlo::diffuseBranch() {
    ...
    double en_old, en_new, w_m, pf, eff_weight, rn;
    int M = 0, h = 0;
    Walker **tmp;
...
int main() {
    ...
    M->prms[1] = 0.;
    for( M->prms[0] = 0.4; M->prms[0] <= 0.4; M->prms[0] += STEPa ) {
        //for( M->prms[1] = -0.5; M->prms[1] <= 0.5; M->prms[1] += STEPb ) {
